/* ==========================================================================
   HYBRID DENTAL CLINIC — MAIN JAVASCRIPT
   ========================================================================== */

/* ===========================================================================
   ⚙️  CLINIC CONFIG — edit these in ONE place (no other code changes needed)
   ...
   =========================================================================== */
window.HDC = {
  WHATSAPP: '2348000000000',                 // TODO: replace with the clinic's real WhatsApp number
  NAME: 'Hybrid Dental Clinic',
  GREETING: 'Hello Hybrid Dental Clinic 👋'  // prepended to every WhatsApp message
};
...
(function () {
  ...
  /* --------------------------------------------------------------------------
     0. WHATSAPP — build wa.me links with service-specific pre-filled messages
     Any element with [data-wa] becomes a WhatsApp link. Use [data-wa-msg] for a
     custom message, or [data-wa-service] to auto-build a booking message.
     -------------------------------------------------------------------------- */
  const WA_NUMBER = (window.HDC && window.HDC.WHATSAPP) || '';
  const WA_GREETING = (window.HDC && window.HDC.GREETING) || 'Hello';

  function waLink(message) {
    return `https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(message || '')}`;
  }
  window.HDC_waLink = waLink;
  ...
  document.querySelectorAll('[data-wa]').forEach(el => {
    ...
    if (custom) {
      ...
      msg = `${WA_GREETING}, I'd like to book a *${service}* appointment. Please share the next available date and the price range. Thank you.`;
    } else {
      msg = `${WA_GREETING}, I'd like to book a dental appointment. Please help me with the next available time.`;
    }
  ...
  function showToast(message, type = 'success') {
    ...
    Object.assign(toast.style, {
      ...
      background: type === 'success' ? '#134B70' : '#D64550',
  ...
  function initBooking(root) {
    ...
    /* Build a clean WhatsApp message from the collected booking state */
    function buildBookingWa() {
      const greeting = (window.HDC && window.HDC.GREETING) || 'Hello';
      const lines = [
        `${greeting}, I'd like to book an appointment:`,
        '',
        `• Treatment: ${state.service || '—'}`,
        `• Preferred dentist: ${state.doctor || 'No preference'}`,
      ...
      lines.push('', 'Please confirm if this works. Thank you!');
      return lines.join('\n');
    }
    ...
    if (confirmBtn) confirmBtn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        renderSummary();
        // Wire the success-screen WhatsApp button to the full booking details
        const waBtn = root.querySelector('[data-booking-wa]');
        if (waBtn && window.HDC_waLink) {
          waBtn.setAttribute('href', window.HDC_waLink(buildBookingWa()));
